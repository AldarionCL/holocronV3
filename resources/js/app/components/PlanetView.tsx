...
export default function PlanetView() {
  ...
  const [travelingToSector, setTravelingToSector] = useState<string | null>(null);
  ...
  const handleSectorClick = (sectorId: string) => {
    // Iniciar viaje terrestre al sector
    setTravelingToSector(sectorId);
  };

  const handleTravelComplete = () => {
    if (travelingToSector) {
      navigate(`/system/${systemId}/body/${bodyId}/sector/${travelingToSector}`);
      setTravelingToSector(null);
    }
  };
  ...
  if (travelingToSector) {
    const targetSector = planetSectors.find(s => s.id === travelingToSector);
    return (
      <GroundTravelModal
        fromName={body.name}
        toName={targetSector?.name || "Sector"}
